...
class BASWebSocketClient:
    ...
    def __init__(
        ...
        Args:
            ...
            max_reconnect_attempts: Maximum connect attempts before raising
                ...
                that crashes the broker service. (default: 5)
    ...
    async def connect(self) -> None:
        ...
        connection storm that previously OOM'd the broker service.
        ...
        async with self._connect_lock:
            if self.is_connected and self.connection is not None:
                log.debug("BAS WebSocket already connected, skipping connect()")
                return
            ...
            for attempt in range(1, self.max_reconnect_attempts + 1):
                ...
                try:
                    ...
                    self._backoff = self.initial_backoff
                    log.info(
                        f"BAS WebSocket connection established | "
                        f"account_id={self.account_id}"
                    )
                    return
                    ...
                    last_error = e
                    log.error(
                        f"BAS connect attempt {attempt}/{self.max_reconnect_attempts} "
            ...
            raise ConnectionError(
                f"BAS WebSocket failed to connect after "
        ...
        try:
            while self.connection:
                try:
                    ...
                    log.warning("BAS WebSocket receive timeout")
                    ...
                    if e.rcvd and "Replaced by new connection" in str(e.rcvd.reason):
                        log.info("BAS connection replaced (server idempotency), stopping gracefully")
                        self.is_connected = False
                        return
    ...
    def _schedule_reconnect(self) -> None:
        ...
        silent reconnect storm previously crashed the broker service.
        """
        self.is_connected = False

        if not self.auto_reconnect:
            log.info(
                "BAS auto_reconnect disabled — marking client disconnected; "
                "call connect() explicitly to reconnect"
            )
            return

        if self._reconnecting:
            log.debug("BAS reconnect already scheduled, skipping duplicate trigger")
            return

        if self._reconnect_task is not None and not self._reconnect_task.done():
            log.debug("BAS reconnect task already running, skipping duplicate trigger")
            return
